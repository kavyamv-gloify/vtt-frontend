  ...
  useEffect(() => {
    ...
    driverTopicList?.map((re) => {
      ...
      tempo.push({
        ...
        defaultPenalty: re?.defaultPenalty,
      });
      // });
    });
            ...
            onClick={() => {
              ...
              driverList_t?.map((elm) => {
                if (driverVal?.value == elm.id) {
                  dd.driverName = elm.firstName + ' ' + elm.lastName;
              ...
              // // for (const [key, value] of Object.entries(tempo)) {
              //     let postData = []
              //     driverTopicList?.map(el=>{
              //         let t = el;
              //         t.complianceSubTopicList = tempo[t.id]
              //         t.status = topicStatus[t.id] || t.status;
              //         postData.push({
              //             driverId: driverVal?.value,
              //             complianceTopic: t
              //         })
              //     })
              //     axios.post(Api.baseUri + '/user-reg/driver-reg/update-driver-topics', postData).then(res => {
              //         if(res?.data?.status == '200'){
              ...
              //     })
              //     // postData.push({
              //     //   driverId: driverList_t[value?.split('_')[0]]?.id,
              //     //   complianceTopic: { ...driverTopicList[value?.split('_')[2]], status: value?.split('_')[1] }
              //     // })
              // //   }
              // // axios.post()
            }}
              ...
              >
                ...
                <TableCell component='th' scope='row'>
                  <div
                    ...
                    onClick={() => {
                      setWaiveOffAmt({...waiveOffAmt, [row?.sid]: '0'});
                      if (topicStatus[row?.tid] != 'MET') {
                        setTopicStatus({
                          ...topicStatus,
                          [row?.tid]: 'WAIVEOFF',
                        });
