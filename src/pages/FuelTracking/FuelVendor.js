...
const FuelVendor = ({close}) => {
  ...
  function getVendorDriver() {
    axios
      .get(
        Api.baseUri +
          `/user-reg/driver-reg/driverListingByVendorId?page=0&size=1000&vendorId=${user?.userList?.profileId}`,
      )
      .then((res) => {
  ...
  function getVendorVehicle() {
    axios
      .get(
        Api.baseUri +
          `/user-reg/vehicle-reg/vehicleListingByVendorId?page=0&size=1000&vendorId=${user?.userList?.profileId}`,
      )
      .then((res) => {
        if (res?.data?.status == '200') {
          console.log('res', res);
  ...
  useEffect(() => {
    if (user?.userList?.userRole == 'VENDOR') {
      getVendorDriver(user?.userList?.profileId);
      getVendorVehicle(user?.userList?.profileId);
    }
  }, []);
  let template = {
    layout: {
  ...
  async function handleSubmitVendor(val) {
    let invoiceDoc = '';
    let odoMeterDoc = '';
    let indentDoc = '';
    ...
    postData.vendorName = user?.userList?.userName;
