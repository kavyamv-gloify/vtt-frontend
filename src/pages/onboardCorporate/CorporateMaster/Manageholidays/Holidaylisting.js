...
const fieldList = [
  ...
  {title: 'Holiday Type', value: 'type'},
  {title: 'Holiday Date', value: 'holidayDate'},
  // {title: 'Holiday Year', value: 'year'},
    ...
    <>
      <Grid container spacing={2} sx={{mb: 6}} className='page-header-second'>
        <Grid item xs={4} md={4} sm={3}>
          <CustomLabel labelVal='Holidays' variantVal='h3-underline' />
        </Grid>
        <Grid item xs={8} md={4} sm={3}>
          <QuickSearchPage
            masterKey='holidayName'
            module={'OfficeHoliday'}
            searchClicked={true}
            searchUrl={'/employee-reg/filter'}
            displayFields={['holidayName', 'type']}
            filterRes={filterRes}
          ...
          <Box display='flex' justifyContent='flex-end'>
            <div className='left-seperator'>
              ...
              {myActions?.includes('Create') && (
                <AppTooltip placement={'top'} title={'Add New Holiday'}>
                  <img
                    src='/assets/images/title-icon/add holiday.svg'
                    className='title-icons-mui'
                    onClick={(e) => {
                      setMyId('create');
                    }}
            ...
            {myActions?.includes('Download And Upload') && (
              <ExcelContainer
                downloadFile={'Holiday'}
                downloadURL={'/user-reg/siteoffice-holyday/download'}
                getHeadersUrl={'/user-reg/siteoffice-holyday/headerdata'}
                downloadTempURL={
                  '/user-reg/siteoffice-holyday/download-template'
                }
                uploadURL={'/user-reg/siteoffice-holyday/import-excel'}
              />
            )}
          </Box>
        </Grid>
            ...
            (rd) =>
              myActions?.includes('Edit') && {
                ...
                onClick: (event, rowData) => {
                  if (rowData.status == 'ACTIVE') handleClickView(rowData);
                },
              },
            (rd) =>
