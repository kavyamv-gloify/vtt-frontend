...
import BtcVolumeCurrency from './BTC-chart';
...
import {elementSize} from '@smart-table/utils/common-values';
...
const CorporateAdmindashboard = () => {
  ...
  // const prevName = usePrevtripByStatusious(temAllTrips);
    ...
    if (d?.shiftId || d?.tripCategory == 'ADHOCTRIP') {
      ...
      axios
        ...
        .then((res) => {
            ...
            meme.routePsDetails?.map((elements) => {
              ...
              if (elements.passType != 'ESCORT') {
                emp_boarding_stats_data.push({
                  ...
                  'Trip Type': elements?.tripType,
                  'Trip Id': elements?.tripCode,
                  Status:
          ...
          // console.log(expectedDelayedData, 'ellellellellellellellell');
          ...
          let vehicleArr = [];
          console.log('tem_all_trip11111', tem_all_trip);
          tem_all_trip?.map((ele, ind) => {
            ele?.vehicletypeList?.map((_el) => {
              vehicleArr.push({
                vehicleType: _el?.vehicleType,
                noOfPassenger: ele?.routePsDetails?.length,
                selectedInd: _ind,
                minCapacityExcludingDriver: _el?.minCapacityExcludingDriver,
                maxCapacityExcludingDriver: _el?.maxCapacityExcludingDriver,
              });
            });
            if (!ele?.vehicletypeList?.length) {
              vehicleArr.push({
                vehicleType: 'NA',
                noOfPassenger: ele?.routePsDetails?.length,
              });
          ...
          console.log(
            "_.groupBy(vehicleArr, 'vehicleType')",
            _.groupBy(vehicleArr, 'vehicleType'),
          );
          ...
          let i = 0;
          let excelDownVendor = [];
          // console.log('groupByVendor', groupByVendor);
          for (const [key, value] of Object.entries(groupByVendor)) {
            if (key && key != 'null' && key != 'undefined' && value.length) {
              ...
              // console.log(
              //   groupByVType,
              //   'groupByVTypegroupByVTypegroupByVTypegroupByVTypegroupByVTypegroupByVTypegroupByVTypegroupByVType',
              // );
            ...
            // console.log('groupByShiftId', groupByShiftId);
        ...
        <Grid container spacing={4}>
                                ...
                                res?.data?.data?.map((elem) => {
                                  tem_ota_otd_trip_data.push({
                                    ...
                                    'Office Arrival/Departure Time':
                                      (elem.tripType == 'UPTRIP'
                                        ? elem.actualTripCompletionTimeStr
                                        : elem.actualTripStartTimeStr
                                      )?.split(' ')[1],
                                    'First point on time': 'Y',
                                    // 'Buffer Time': '0',
                                    // 'Delay Status': 'NO',
            ...
            <div className='card-normal min-h-300'>
              ...
              <div style={{padding: '14px'}}>
                <Pie
                  ...
                  labels={['Not Started', 'Inprogress', 'Completed', 'Others']}
          ...
          <Grid item xs={12} sm={6}>
            <div className='card-normal min-h-300'>
                ...
                <div>
                  ...
                  {myActions?.includes('Download And Upload') && (
                    <FileDownloadIcon
                      ...
                      onClick={() => {
                        exportData.trip_stats_data;
                        if (_.isEmpty(exportDataVendor)) {
                          toast.error('No Data found!');
                        } else {
                          downDoc.downloadReport(
                            {
                              Trip_Vendor_Distribustion: exportDataVendor,
                            },
                            'Trip Vendor Distribution',
                          );
                        }
                      }}
                      style={{position: 'absolute', right: '8px'}}
              ...
              <div style={{padding: '14px'}}>
                <CustomActiveShapePieChart data={tripByVendor} />
              </div>
            </div>
          </Grid>
        </Grid>
