...
import {ConstructionOutlined, PostAdd, Watch} from '@mui/icons-material';
...
const NewUserGuide = ({data, close}) => {
  ...
  async function onSubmit(val) {
    // console.log('val', val);
    // console.log('newFileName', newFileName?.length);
    // console.log('name', newFileName[0]?.name);
    let postData = {};
    ...
    postData.fileName = newFileName[0]?.name;
    postData.description = title?.description;
    postData.id = data?.id;
    postData.status = 'ACTIVE';
    if (newFileName?.length) {
      postData.doc = await getFileName(newFileName);
    }
    if (newFileName?.length == 0) {
      postData.doc = val?.doc;
    }
