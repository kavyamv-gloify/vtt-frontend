...
const Create = () => {
  ...
  useEffect(() => {
    ...
    let days = Math.floor(data?.vehicleTotalHours / 24);
    let hour = data?.vehicleTotalHours % 24;
    setData({...data, periodDays: days, periodHours: hour});
        ...
        selFuelType?.map((fuel) => {
          selTripType?.map((tripType) => {
            postData.push({
              ...
              vehicleTotalHours:
                Number(Number(val?.periodDays) * 24) + Number(val?.periodHours),
              vehicleTotalDistanceInKM: val?.vehicleTotalDistanceInKM,
            });
          });
        });
      });
      ...
      : axios
          ...
          .then((res) => {
            if (res?.data?.status == '200') {
              ...
              localStorage.setItem('Tab_value', 3);
              navigate('/rate-card-list');
            } else {
              toast.error(res?.data?.message || 'Something went wrong.');
            }
              ...
              {(params?.id == 'CREATE' || data) && (
                <form onSubmit={handleSubmit(onSubmit)}>
                  <Grid container spacing={6}>
                    ...
                    <Grid item xs={6} sm={6} md={6}>
                      <h5>
                        Fixed Cost<span style={{color: 'red'}}>*</span>
                      </h5>
                      <TextField
                        // error
                        {...register('basePrice', {
                    ...
                    <Grid item xs={6} sm={6} md={6}>
                      ...
                      <TextField
                        // error
                        {...register('vehicleTotalDistanceInKM', {
                          required: 'This is a  mandatory field',
                          pattern: {
                            value: regex.numReg,
                            message: 'Please enter valid price.',
                      ...
                      <Grid container spacing={2}>
                        <Grid item xs={6} sm={6} md={6}>
                          <TextField
                            ...
                            // onChange={(e, v) => { setInputField({ ...inputfield, fuelfrom: e?.target?.value || data?.fuleRangeFrom }) }}
                            id='outlined-error-helper-text'
                            size='small'
                            fullWidth
                          />
                        ...
                        <Grid item xs={6} sm={6} md={6}>
                          <TextField
                            {...register('periodHours', {
                              ...
                              // validate: {
                              //     lessThan: v => Number(v) > Number(watch("fuleRangeFrom"))
                              // }
                            })}
                            // onChange={(e, v) => { setInputField({ ...inputfield, fuelTo: e?.target?.value }) }}
                            // error
                            id='outlined-error-helper-text'
                            // label="Error"
                            size='small'
