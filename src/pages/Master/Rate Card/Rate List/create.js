...
const slabList = [
  {title: 'Slab 1', value: 'slab1'},
  {title: 'Slab 2', value: 'slab2'},
  {title: 'Slab 3', value: 'slab3'},
  {title: 'Slab 4', value: 'slab4'},
  {title: 'Slab 5', value: 'slab5'},
  {title: 'Slab 6', value: 'slab6'},
  {title: 'Slab 7', value: 'slab7'},
  {title: 'Slab 8', value: 'slab8'},
  {title: 'Slab 9', value: 'slab9'},
  {title: 'Slab 10', value: 'slab10'},
];
...
const Create = () => {
  ...
  const [selSlabName, setSelSlabName] = useState({});
  ...
  async function getLists() {
    ...
    if (resp?.data?.status == '200') {
      ...
      resp?.data?.data?.body?.VehicleTypeList?.map((el) => {
        tempVehicleType.push({
          title: el?.vehicleType + '-' + el?.vehicleOccupancy,
          value: el?.id,
        });
      });
  ...
  useEffect(() => {
    ...
    slabList?.map((el) => {
      if (el.value == data?.slabName) setSelSlabName(el);
    });
              ...
              {(params?.id == 'CREATE' || data) && (
                <form onSubmit={handleSubmit(onSubmit)}>
                  <Grid container spacing={6}>
                    ...
                    <Grid item xs={6} sm={6} md={6}>
                      <h5>
                        Slab Name <span style={{color: 'red'}}>*</span>
                      </h5>
                      <Autocomplete
                        // multiple
                        options={slabList}
                        value={_.isEmpty(selSlabName) ? null : selSlabName}
                        error={subClicked && _.isEmpty(selSlabName)}
                        getOptionLabel={(option) => option.title}
                        disablePortal
                        size='small'
                        onChange={(e, data) => {
                      ...
                      />
                      <p style={{fontSize: '12px', color: 'red'}}>
                        {subClicked && _.isEmpty(selSlabName)
                          ? 'Please select Slab Name.'
                          : ''}
                      </p>
                    </Grid>
                    <Grid item xs={6} sm={6} md={6}>
                      <h5>
                        Distance Range (From and To){' '}
                        <span style={{color: 'red'}}>*</span>
                      </h5>
                      <Grid container spacing={2}>
                        <Grid item xs={6} sm={6} md={6}>
                        ...
                        <Grid item xs={6} sm={6} md={6}>
                          <TextField
                            {...register('distanceTo', {
                              ...
                              // validate: {
                              //     lessThan: v => (v) > ((watch("distanceFrom"))),
                              //     message: "Distance To should be less than Distance From"
                              // }
                            })}
                            // error
                            id='outlined-error-helper-text'
