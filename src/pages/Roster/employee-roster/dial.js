...
const Dial = ({
  ...
  useEffect(() => {
    ...
    selectedDate?.map((e) => {
      temArr.push(
        moment(new Date(e?.split('/')?.reverse()?.join('-'))).format('DD MMM'),
      );
    });
  ...
  useEffect(() => {
    ...
    if (employee?.length) {
      employee?.map((em) => {
        if (selectedEmployee?.includes(em?.id)) {
          temsel.push({
              ...
              : [
                  {
                    ...
                    // defaultValue: tempdatearray?.length ? tempdatearray[0] : '',
                    minmax: 'custom',
                  ...
                  },
                  {
                    ...
                    // defaultValue: tempdatearray?.length ? tempdatearray[6] : '',
                    minmax: 'custom',
                    minCustomDate: new Date().setDate(new Date().getDate() + 1),
                    maxCustomDate: new Date().setDate(
                      new Date().getDate() +
  ...
  function handleDirectSubmit(val) {
    let dateee = [];
    if (!selectedDate?.length) {
  ...
  function handleSubmit(val) {
    setShowbtn(false);
    if (
      rosterSettings?.allowManagerApproval?.toUpperCase()?.trim() == 'NO' &&
      rosterSettings?.allowAdminApproval?.toUpperCase()?.trim() == 'NO'
    ) {
      handleDirectSubmit(val);
      return;
    }
    let dateee = [];
    // return;
    ...
    if (!selectedDate?.length) {
      ...
      daylist?.map((da) => {
        ...
        if (
          ...
          // if(!val?.data?.holidays?.includes(moment(da).format("YYYY-MM-DD"))){
          // if (myweekoffs?.includes(weeks[new Date(se?.split("/")?.reverse()?.join('-'))?.getDay()]?.value)) {
          if (myweekoffs?.includes(weeks[da.getDay()]?.value)) {
            dateee.push({
              date: moment(da).format('YYYY-MM-DD'),
              weekOff: shiftFor == 'BOTH' ? 'YES' : 'NO',
            ...
            dateee.push({
              date: moment(da).format('YYYY-MM-DD'),
              weekOff: val?.data?.holidays?.includes(
                moment(da).format('YYYY-MM-DD'),
              )
                ? 'YES'
                : 'NO',
            });
      ...
      selectedDate?.map((eb) => {
        let se =
          eb?.split('/')[1] + '/' + eb?.split('/')[0] + '/' + eb?.split('/')[2];
        let inTime;
        let outTime;
        loginShiftList?.map((lt) => {
          if (lt?.value == val?.data?.loginshift) {
        ...
        let temCheck1 = compareTime(
          new Date(eb?.split('/')?.reverse()?.join('-')),
          inTime,
          'LOGIN_MODIFY',
        );
        ...
        if (
          ...
          // if(!val?.data?.holidays?.includes(moment(eb).format("YYYY-MM-DD"))){
          if (
            myweekoffs?.includes(
              weeks[new Date(eb?.split('/')?.reverse()?.join('-'))?.getDay()]
                ?.value,
            )
          ) {
            dateee.push({
              date: moment(se).format('YYYY-MM-DD'),
              weekOff: shiftFor == 'BOTH' ? 'YES' : 'NO',
            });
          } else {
            dateee.push({
    ...
    let outtime = '';
    shiftList?.map((r) => {
      if (r?.value2?.id == val?.data?.loginshift) {
        intime = r?.value2?.login;
      }
      if (r?.value2?.id == val?.data?.logoutshift) {
        outtime = r?.value2?.logout;
      }
    });
    let postData = {
      reqIds: [],
      ...
      officeId: MyData?.officeId,
      officeName: MyData?.officeName,
      departmentId: MyData?.departmentId[0],
      departmentName: MyData?.department,
      employeeId: MyData?.id,
      employeeName: MyData?.employeeFullName,
      employeeCode: MyData?.employeeCode,
      gender: MyData?.gender,
      emailId: MyData?.emailId,
      manegerId: MyData?.managerId,
      manegerName: MyData?.managerName,
      loginShiftId:
        val?.data?.loginshift != 'null' ? val?.data?.loginshift : null,
      logoutShiftId:
        val?.data?.logoutshift != 'null' ? val?.data?.logoutshift : null,
    ...
    axios
      .post(Api?.baseUri + '/user-reg/roaster/save-roaster-req', postData)
      .then((res) => {
        setShowbtn(true);
        if (res?.data?.status == '200') {
          toast.success(res?.data?.message ?? 'Roster applied successfully.');
          setOpen(false);
          openDialFun(false);
        } else {
          toast.error(res?.data?.message ?? 'Something went wrong.');
