...
const CreateRoster = () => {
  ...
  useEffect(() => {
    ...
    permissionCheck?.map((el) => {
      if (el.subModuleName == 'RosterEmployee') sub_mod = el;
    });
  ...
  const fetchPendingRoster = () => {
    axios
      .get(Api?.baseUri + '/user-reg/roaster/get-team-roaster-req-empId')
      .then((res) => {
        let myobj = {};
        res?.data?.data?.map((rem) => {
          rem?.dates?.map((el) => {
            myobj[moment(el?.date).format('DD/MM/YYYY')] =
              (rem?.loginTime || 'OFF') + '<<==>>' + (rem?.logoutTime || 'OFF');
          });
        });
    ...
    axios
      ...
      .then((res) => {
        ...
        let postdata = {
          ...
          loginShiftId: login != 'null' ? login : null,
          logoutShiftId: logout != 'null' ? logout : null,
          dates: [{date: moment(date).format('YYYY-MM-DD'), weekOff: 'NO'}],
          employeeIds: [user?.userList?.profileId],
        };
        axios
      ...
      <Grid container spacing={2} sx={{mb: 8}} className='page-header-second'>
        ...
        <Grid item xs={6}>
          <Box display='flex' justifyContent='flex-end'>
            {rosterSettings?.empManageRoaster?.toUpperCase()?.trim() == 'YES' &&
                    ...
                    >
                      <div style={{marginLeft: '2rem'}}>
                        <div
                          ...
                          onClick={(e) => {
                            if (
                              rosterSettings?.empManageRoaster
                                ?.toUpperCase()
                                ?.trim() != 'YES'
                            )
                              return;
