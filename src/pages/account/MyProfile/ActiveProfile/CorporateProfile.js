...
const EditForm = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      if (
        ...
        user?.userList?.userStatus != 'ACTIVE'
      ) {
        navigate('/dashboard');
      }
      ...
      tempo.tempcompanyAddressOne =
        tempo?.companyAddress?.addressName?.split('++')[0];
      tempo.tempcompanyAddressOneAREA =
        tempo?.companyAddress?.addressName?.split('++')[1];
              ...
              fields: [
                {
                  ...
                  //   message: 'Please enter valid company name and below 50 characters'
                  // },
                  // validationProps: {
  ...
  const handleSubmit = async (values) => {
    ...
    if (values.button.toUpperCase() === 'SUBMIT') {
      ...
      axios({
        method: 'post',
        url: api.onBoardCorporate.changeRequest,
  ...
  //     let allElem = {};
  //     let tem = values?.data;
  //     tem.bankCode = values?.data?.bankName?.code;
  //     tem.bankName = values?.data?.bankName?.name;
  //     Object.keys(tem).map((key) => {
  //       //
  //       //  )
  //       if (typeof tem[key]?.[0]?.name == 'string') {
  ...
  //         }
  //       } else if (Object.keys(tem[key]).length !== 0) {
  //         if (tem[key] !== "" || tem[key] !== null || tem[key] !== undefined || Object.keys(tem[key]).length !== 0) {
  //           allElem = {
  //             ...allElem,
  //             [key]: tem[key]
  //           }
  ...
  //         //handle success
  //         if (response?.data?.status == '200') {
  //           toast.success(response?.data?.message, "Submited successfully");
