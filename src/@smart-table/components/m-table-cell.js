...
export default class MTableCell extends React.Component {
  getRenderValue() {
    ...
    else if(this.props.columnDef.type === "arraycount"){
      if (typeof this.props.value == 'object' && this.props.value?.length){return this.props.value?.length}
      else{return 'NA'}
    }
    else if(this.props.columnDef.type === "objectmerge"){
      if (typeof this.props.value == 'object'){
        if(this.props.columnDef?.secondaryfields){ 
          let tem = this.props.columnDef.secondaryfields?.split(",");
          let temString = '';
          tem?.map((e,ind)=>{
            let isMulti = (tem?.length>1 && ind+1 != tem?.length) ? ", " : ''
          temString = temString+this.props.value[e]+isMulti
          })
