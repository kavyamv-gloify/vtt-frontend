...
export default class MaterialTable extends React.Component {
  ...
  setDataManagerFields(props, isInit) {
    ...
    if (this.isRemoteData(props)) {
      ...
      this.dataManager.changeApplySort(true); // Please make it false for getting sorted data from backend side // CHANGES DONE BY RAJRANJAN FOR FROMTEND SORTING
    } else {
          ...
          <TableFooter style={{display: 'grid'}}>
            <TableRow>
              <props.components.Pagination
                ...
                style={{
                  float:
                    props && props.theme && props.theme.direction === 'rtl'
                      ? ''
                      : 'right',
                  overflowX: 'auto',
                }}
  ...
  renderTable = (props) => (
    <TableContainer sx={{maxHeight: 'calc(100vh - 215px)'}}>
      <Table
        stickyHeader
        style={{
          tableLayout:
